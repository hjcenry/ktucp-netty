...
import com.hjcenry.internal.ReItrLinkedList;
import com.hjcenry.internal.ReusableListIterator;
...
public class Kcp implements IKcp {
     ...
     * 数据统计
     */
    protected volatile Snmp snmp = new Snmp();
    ...
    private static int encodeSeg(Kcp kcp, ByteBuf buf, Segment seg) {
        ...
        Snmp.snmp.addOutSegments(1);
        kcp.snmp.addOutSegments(1);
    ...
    public int send(ByteBuf buf) {
        ...
        if (count > 255) {
            // Maybe don't need the condition in stream mode
            return -2;
        }
    ...
    public int input(ByteBuf data, boolean regular, long current) {
        ...
        while (true) {
            ...
            //最后收到的来计算远程窗口大小
            if (regular) {
                //更新远端窗口大小删除已确认的包，una以前的包对方都收到了，可以把本地小于una的都删除掉
                this.rmtWnd = wnd;
            }
            ...
            switch (cmd) {
                ...
                case IKCP_CMD_PUSH: {
                    ...
                    if (regular && repeat) {
                        Snmp.snmp.addRepeatSegments(1);
                        this.snmp.addRepeatSegments(1);
                    }
                    if (LOG.isDebugEnabled()) {
        ...
        // 统计
        Snmp.snmp.addInSegments(inSegs);
        this.snmp.addInSegments(inSegs);

        if (flag && regular) {
            int rtt = itimediff(uintCurrent, latest);
            if (rtt >= 0) {
                //收到ack包，根据ack包的时间计算srtt和rto
                updateAck(rtt);
            }
    ...
    public long flush(boolean ackOnly, long current) {
        ...
        if (lostSegs > 0) {
            // 统计
            Snmp.snmp.addLostSegments(lostSegs);
            this.snmp.addLostSegments(lostSegs);
        }
        if (fastRetransSegs > 0) {
            // 统计
            Snmp.snmp.addFastRetransmitSegments(fastRetransSegs);
            this.snmp.addFastRetransmitSegments(fastRetransSegs);
            sum += fastRetransSegs;
        }
        if (earlyRetransSegs > 0) {
            // 统计
            Snmp.snmp.addEarlyRetransmitSegments(earlyRetransSegs);
            this.snmp.addEarlyRetransmitSegments(earlyRetransSegs);
            sum += earlyRetransSegs;
        }
        if (sum > 0) {
            // 统计
            Snmp.snmp.addRetransmitSegments(sum);
            this.snmp.addRetransmitSegments(sum);
        }
        // update ssthresh
        if (!nocwnd) {
            if (change > 0) {
