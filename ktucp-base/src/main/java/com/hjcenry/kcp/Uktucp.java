...
public class Uktucp extends DefaultAttributeMap {
    ...
    public Uktucp(KtucpOutput output,
        ...
        if (channelConfig.isCrc32Check()) {
            headerSize += ChannelConfig.CRC_32_SIZE;
        }
    ...
    protected void input(ByteBuf data, long current) throws IOException {
        Snmp.snmp.addInPackets(1);
        Snmp.snmp.addInBytes(data.readableBytes());
        getSnmp().addInPackets(1);
        getSnmp().addInBytes(data.readableBytes());
     ...
     * 线程安全的<br/>
     *
     * @param object 写ByteBuf时不要调用release，内部会调用，否则可能抛出{@link io.netty.util.IllegalReferenceCountException}异常
    ...
    public KtucpListener getKcpListener() {
    ...
    public void resetForceUseNet() {
        User user = user();
        user.setForceUseNetId(INet.NO_USE_FORCE_NET_ID);
    }
     ...
     * @return 修改结果
     */
    public boolean changeForceUseNet(int netId) {
        User user = user();
        if (!user.getUserNetManager().containsNet(netId)) {
            return false;
        }
        user.setForceUseNetId(netId);
        return true;
    }
