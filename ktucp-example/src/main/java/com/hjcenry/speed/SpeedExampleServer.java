...
public class SpeedExampleServer extends SimpleKtucpListener<ByteBuf> {
    ...
    protected void handleReceive0(ByteBuf cast, Uktucp uktucp) throws Exception {
        ...
        if (now - start >= 1000) {
            System.out.println("耗时 :" + (now - start) + " 接收数据: " + (Snmp.snmp.getInBytes().doubleValue() / 1024.0 / 1024.0) + "MB" + " 有效数据: " + inBytes / 1024.0 / 1024.0 + " MB");
            System.out.println(Snmp.snmp.getBytesReceived().doubleValue() / 1024.0 / 1024.0);
